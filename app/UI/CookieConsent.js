...
const CookieConsent = () => {
  ...
  // Ładowanie Microsoft Clarity (tylko gdy zgoda na analytics)
  ...
  // Aktualizacja zgody w Google Consent Mode (dla GTM/Google Ads/Analytics)
  ...
  useEffect(() => {
    ...
    if (storedConsent) {
      ...
      // Clarity ładujemy tylko jeśli była zgoda
      if (parsed.analytics_storage === "granted") {
        loadClarityScript();
        setTimeout(() => sendClarityConsent(parsed), 1000);
      }
  ...
  const saveConsent = (newConsent) => {
    ...
    // 2. Obsłuż Clarity osobno (bo nie ma Consent Mode)
    if (newConsent.analytics_storage === "granted") {
      loadClarityScript();
      setTimeout(() => sendClarityConsent(newConsent), 1000);
    } else {
