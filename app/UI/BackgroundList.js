  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto text-center mb-8 md:mb-14">
        <h3
          className={`text-2xl md:text-3xl xl:text-4xl font-bold mb-4 ${textColor}`}
        >
          {title}
        </h3>
        {text && (
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 max-w-5xl mx-auto">
        {items.map((item, index) => (
          ...
          >
            {/* Zmniejszona ikona na mobile (w-5), większa na desktopie (md:w-6) */}
            <CheckCircle2 className="w-5 h-5 md:w-6 md:h-6 mt-0.5 text-blue-600 shrink-0" />

            <span
              className={`text-sm md:text-lg font-medium leading-snug md:leading-normal ${textColor}`}
            >
              {item}
            </span>
          </li>
